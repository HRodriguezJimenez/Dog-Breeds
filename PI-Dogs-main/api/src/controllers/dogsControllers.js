...
const getDogByName = async (name) => {
    ...
    if (!dogByName.length) throw new Error(`No existe ninguna raza con el nombre: ${name}`);
...
const getDogById = async (id) => {
    ...
    if (!dogById.length) throw new Error(`No existe esta raza en específico: ${id}`);
...
const createDogDB = async (
    ...
    temperaments,) => {
        ...
        if (breedByName) {
            throw new Error(`Ya existe una raza con este nombre: ${name}`);
        }    
